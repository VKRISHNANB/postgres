...
exec sp_columns 'Books_venkat' -- column list
...
exec sp_help 'Books_venkat'
-- Sp_helptext :  Prints the text of a rule, a default, or 
--                an unencrypted stored procedure,
--                user-defined function, trigger, or view
exec sp_helptext 'sys.sp_addlogin'
exec sp_helptext 'sys.sp_columns'
exec Sp_helptrigger 'Books_venkat'
exec sp_helpindex 'Books_venkat'
exec Sp_tables
...
select loginame 
from sys.sysprocesses 
where DB_NAME(dbid) ='Northwind' and dbid>0

exec sp_helptext sysprocesses
...
SELECT	'[' + SCHEMA_NAME([Schema_id]) + '].' + '['+OBJECT_NAME(ST.[object_id]) + 
']' [Table_Name],'[' + SIC.Name + ']' [Column_Name], Seed_Value,
	Increment_Value, ISNULL(last_Value,0)last_Value
FROM	Sys.Tables ST
